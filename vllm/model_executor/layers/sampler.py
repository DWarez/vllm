...
from vllm.sampling_params import DynamicTemperatureConfig, SamplingType
...
# Dynamic temperature
class DynamicTemperatureTracker:
    """Tracks generation steps for dynamic temperature calculation."""
    ...
    def get_current_temperature(
        ...
        """Calculate temperature for current generation step."""
        if config is None:
            return default_temp

        current_step = seq_id
        decay_steps = config.decay_steps if config.decay_steps else max_tokens

        if current_step >= decay_steps:
            return config.end_temp
        ...
        if config.decay_type == "linear":
            temp = (
                config.start_temp
                + (config.end_temp - config.start_temp) * progress
            )
        elif config.decay_type == "exponential":
            decay_rate = math.log(config.end_temp / config.start_temp)
            temp = config.start_temp * math.exp(decay_rate * progress)
        else:
...
class Sampler(nn.Module):
    ...
    def __init__(self):
        ...
        self.dynamic_temp_tracker = DynamicTemperatureTracker()
    ...
    def _create_dynamic_temperature_tensor(
        self,
        sampling_metadata: SamplingMetadata,
        default_temperatures: torch.Tensor,
    ) -> torch.Tensor:
        """
        Create temperature tensor with dynamic temperatures where configured.

        Args:
            sampling_metadata: Metadata containing sequence information
            default_temperatures: Original temperature tensor from sampling_tensors

        Returns:
            Modified temperature tensor with dynamic temperatures applied
        """
        # Clone the default temperatures
        temperatures = default_temperatures.clone()

        # Get sequence groups and their sampling parameters
        seq_groups = sampling_metadata.seq_groups

        # Track position in the flattened tensor
        position = 0
        ...
        for seq_group in seq_groups:
            ...
            # Check if this group uses dynamic temperature
            if sampling_params.dynamic_temperature is not None:
                # Calculate dynamic temperature for each sequence in the group
                for i, seq_id in enumerate(seq_ids):
                    dynamic_temp = (
                        self.dynamic_temp_tracker.get_current_temperature(
                            seq_id,
                            sampling_params.dynamic_temperature,
                            sampling_params.temperature,
                            sampling_params.max_tokens or 100,
                        )
                    )

                    # Apply to the corresponding position in temperature tensor
                    if position + i < len(temperatures):
                        temperatures[position + i] = dynamic_temp
    ...
    def forward(
        ...
        has_dynamic_temp = any(
            seq_group.sampling_params.dynamic_temperature is not None
            for seq_group in sampling_metadata.seq_groups
        )
        
        if has_dynamic_temp:
            # Create modified temperature tensor with dynamic values
            temperatures = self._create_dynamic_temperature_tensor(
                sampling_metadata,
